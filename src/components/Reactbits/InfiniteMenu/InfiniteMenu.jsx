...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, autoRotate = false) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  #updateProjectionMatrix(gl) {
    ...
    const height = this.SPHERE_RADIUS * 0.36;
  ...
  #onControlUpdate(deltaTime) {
    ...
    // Auto-rotate logic
    if (this.autoRotate && !this.control.isPointerDown) {
      const ROTATE_SPEED = 0.01;
      const axis = vec3.fromValues(0, 1, 0);
      const q = quat.create();
      quat.setAxisAngle(q, axis, ROTATE_SPEED * timeScale);
      quat.multiply(this.control.orientation, q, this.control.orientation);
      quat.normalize(this.control.orientation, this.control.orientation);
    }

    // Only snap when NOT auto-rotating
    if (!this.control.isPointerDown && !this.autoRotate) {
...
export default function InfiniteMenu({ items = [], navigate }) {
  const canvasRef = useRef(null);
  const sketchRef = useRef(null); // 1. Add this line
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      sketchRef.current = sketch; // 2. Save instance to ref
    }
  ...
  // 3. Update autoRotate on the instance when it changes
  useEffect(() => {
    if (sketchRef.current) {
      sketchRef.current.autoRotate = autoRotate;
    }
  }, [autoRotate]);

  useEffect(() => {
    const timer = setTimeout(() => setAutoRotate(false), 5000);
